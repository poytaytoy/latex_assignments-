    ...
    \begin{enumerate}
        ...
        Hence, it remains to prove the claim if $p$ is an odd prime where $p \geq 5$. Let us assume $3$ is a quadratic residue mod $p$. Hence, $(\frac{3}{p})$ = 1. We consider 2 cases on the basis of Theorem 11.4 (c).\\
        ...
        \textbf{Case 2: }If $p \not \equiv -1 \pmod{4}$. Then, $(\frac{3}{p}) = (\frac{p}{3})$ = 1. This implies that $p^{(3-1)/2} = p \equiv 1 \pmod{3}$. Since $p$ is an odd prime, we note that $p \not \equiv 2 \pmod{4}$ or $p \not \equiv 0 \pmod{4}$. This leaves that $p \equiv 1 \pmod{4}$. Since $3$ and $4$ are co-prime, we get that $p \equiv 1 \pmod{12}$. \\

        Considering all the cases, we get that $(\frac{3}{p}) = 1$ if $p = 2, 3$ or $p \equiv \pm 1 \pmod{12}$ For the converse, we assume that $p \equiv \pm 1 \pmod {12}$. For $p \equiv -1 \pmod {12}$, it implies that $p \equiv -1 \equiv 3 \pmod 4$ and $p \equiv -1 \pmod 3$. Since $3 \equiv 3 \pmod 4$ by Theorem 11.4 (c), we note that $(\frac{3}{p}) = -(\frac{p}{3})$. Further more, $-1$ is not a quadratic residue mod $3$, so $ -(\frac{p}{3}) = -(-1) = 1$. Meanwhile, if $p \equiv 1 \pmod {12}$, then $p \equiv 1 \pmod 3$, so $p$ is a quadratic residue mod $3$. In both cases, $3$ is a quadratic residue. \hfill $\square$  \\

        \item 
        For $p = 2$, we note that $-3 \equiv -1 \equiv 1 \pmod{2}$, so it is a quadratic residue. Meanwhile for $p = 3$, $-3 \equiv 0 \pmod{3}$, so it is also a quadratic residue. Hence, we concern ourselves with the case where $p \geq 5$. Let us assume that $-3$ is a quadratic residue mod $p$. This implies that $(\frac{-3}{p}) = (\frac{-1}{p})(\frac{3}{p}) = 1$. In other words, either both $(\frac{-1}{p})$ and $(\frac{3}{p})$ must be equal. Hence: 
        ...
        This implies that $p$ is a square mod $3$. In $\mathbb{F}_3$, only $0$ and $1$ are squares. We can rule out $p = 0$ because $p \neq 3$. Hence, we proved that if $-3$ is a quadratic residue for $p \geq 5$, then $p \equiv 1 \pmod 3$. For the converse, assume that $p \equiv 1 \pmod 3$. By Theorem 11.4 (c), we note that:
        
        $$(\frac{3}{p})(\frac{p}{3}) = (-1)^{(p-1)/2\cdot(3-1)/2} = (-1)^{(p-1)/2}$$

        Note that $(\frac{p}{3}) = 1$, so $(\frac{3}{p}) = (-1)^{(p-1)/2}$. We now examine $(\frac{-3}{p}) = (\frac{-1}{p})(\frac{3}{p}) = (-1)^{(p-1)} = 1$. This implies that $-3$ is a quadratic residue mod $p$.  

        \hfill $\square$  \\

        \item 
        By contradiction, we assume that $p \neq 2$ and $p \not \equiv 1 \pmod{4}$. This implies that $p$ is an odd prime where $2 \nmid (p-1)/2$. We note that $x^2 \equiv -y^2 \pmod{p}$. This implies that $-y^2$ is a quadratic residue mod $p$, so either $(\frac{-y^2}{p}) = 0$ or $1$. If $(\frac{-y^2}{p}) = 1 = (\frac{-1}{p})(\frac{y}{p})(\frac{y}{p})$, we note that $(\frac{-1}{p}) = -1$ since $(p-1)/2$ is odd. This implies that $(\frac{y}{p})^2 = -1$. The values of Legrendre symbols squared is either $0$ or $1$, so $-1$ is impossible thus a contradiction. Meanwhile, if $(\frac{-y^2}{p}) = 0$, then with $(\frac{-1}{p}) = -1$, $(\frac{y}{p}) = 0$. This implies $p \mid y$, but if $y \neq 0$, then $x^2 + y^2 \geq y^2 > p$, a contradiction, and if $y = 0$, then $p = x^2$, contradicting $p$ being prime. In all cases, we reach a contradiction thus $p = 2$ or $p \equiv 1 \pmod{4}$. \hfill $\square$ \\
    ...
    \begin{enumerate}
        ...
        \textbf{Claim: }If $a$ is an integer where $p \nmid a$, then there exists an integer solution $x$ for $x^3 \equiv a \pmod p$. \\
        ...
        \item 
        Let us use $7 \in \mathbb{Z}_3$ and denote $f(x) = x^3 - 19$. We note that $f(7) = 7^3 - 19 = 324$ and that $f'(7) = 3 \cdot (7)^2 = 147$ in $\mathbb{Z}_3$. Note that using the definition from HW6 6 (c):
        
        $$324 = 0 + 0 + 0 + 0 + 1 \cdot 3^4 + 1 \cdot 3^5 + 0 + \cdots$$ 
        
        $$147 = 0 + 1 \cdot 3^1 + 1 \cdot 3^2 + 2 \cdot 3^3 + 1 \cdot 3^4 + 0 + \cdots$$

        This implies that $\nu_3(324) = 4$ and $\nu_3(147) = 1$. In other words, $\nu_3(f(7)) > 2\nu_3(f'(7))$. We can apply Hensel's Lemma and get that there exist a $\alpha \in \mathbb{Z}_3$ where $f(\alpha) = 0$. Take the $k$-th coordinate of $\alpha$ as $a_k$ and we see that $a_k^3 - 19 = 0$ in $\mathbb{Z}/3^k \mathbb{Z}$. This translates to that there exist an integer solution to $x^3 - 19 \equiv 0 \pmod {3^k}$. \hfill $\square$  
    ...
    \begin{enumerate}
        ...
        By contradiction, they are both equal to the form $h(x) = x^m a_{m-1}x^{m-1} + \cdots + a_rx^r$ and $g(x) = x^n + b_{n-1}x^{n-1} + \cdots + b_sx^s$ where all $a_r$ and $b_r$ are non-zeros in $\mathbb{F}_{23}$. We then get $g(x)h(x) = x^{m+n} + \cdots b_sa_rx^{r + s}$. Since $\mathbb{F}_{23}$ is an integral domain, we get that $b_sa_rx^{r + s}$ is a non-zero term, which implies $f(x)$ cannot be equal to a perfect power, a contradiction. If only one of them is equal to a perfect power, where we assume $h(x) = x^m a_{m-1}x^{m-1} + \cdots + a_rx^r$ and $g(x) = x^n$ where $a_r$ is non-zero in $\mathbb{F}_{23}$. This means $h(x)g(x) = x^{m+n} + \cdots a_rx^{r+n}$ and the same contradiction follows with the non-zero term $a_rx^{r+n}$. Hence, both $g(x)$ and $h(x)$ are equivalent to perfect powers in $\mathbb{F}_{23}[x]$. \hfill $\square$  \\
        ...
        This implies that $\nu_3(2025) = 4$ and $\nu_3(69) = 1$. In other words, $\nu_3(f(0)) > 2\nu_3(f'(0))$. We apply Hensel's Lemma and get that there exists an $\alpha \in \mathbb{Z}_3$ where $f(\alpha) = 0$. In other words, we found such root. \hfill $\square$  
